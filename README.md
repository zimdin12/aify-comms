...
Use **managed mode** for the normal persistent team. Start `aify-comms` in each Windows/WSL/Linux environment you want to execute work in, then spawn agents from the dashboard. Managed identities have a saved environment, workspace, runtime, spawn spec, native handle when available, and session history. For terminal-capable runtimes, dashboard sends use a bridge-owned PTY backing so the same runtime can be watched or driven from the browser Console without switching identity modes. The dashboard can restart, stop, compact, or recreate agents without keeping a separate CLI tab open.
...
Normal dashboard chat is live-delivery gated for unreachable targets: offline, stale, stopped, or no-wake agents fail visibly and the message is not stored for a future run. Terminal-capable managed runtimes receive normal sends through their active or newly-started PTY; the browser Console is an attachment to that same backing, not a separate owner. These PTY sends create delivered Work Loop contracts, not queued dispatch runs. Busy non-PTY live targets receive ordinary sends as steer when supported, or as queued/merged next-turn work when steering is not available; the explicit **Queue** action forces next-turn delivery. Required handoffs are repaired automatically when a terminal run finishes without an explicit reply, and the Home page exposes repair/dismiss actions for old issue states.
