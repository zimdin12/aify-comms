...
Managed sessions are **bridge-owned**: the environment bridge that spawned them owns their worker processes. Two consequences follow. First, **restarting `aify-comms` is a clean slate** — the bridge tears down every managed session it owns on shutdown and, on the next start, sweeps any survivors of a crashed predecessor; both are scoped to the agents that bridge owns and never touch resident sessions or another environment's agents. So a restart leaves no orphaned workers holding agents falsely `working`, and managed sessions are re-spawned fresh from their spec rather than inherited. (Run a session **resident** if you want it to persist a restart with its terminal intact.) Second, **`online` means deliverable, not just present** — a managed agent reads `online` only when it has a live claimer behind it (the delivery loop / channel-sidecar that actually receives work), not merely because its gateway answers or a Console is open. A send to a managed agent whose claimer is gone fails fast with an actionable reason instead of queuing against a worker that will never claim it.
