...
Three opt-in settings shape the managed-delivery surface. `insert_messages_via_console=false (the default channel-route mode; earlier name claude_managed_channel_only=false)` switches managed Claude from PTY-input delivery to channel events (`claude-channel.js` claims the dispatch and emits `<channel source="aify-comms-channel" ...>` MCP notifications — same protocol resident Claude already uses). `managed_pty_eager_spawn=true` (paired with `managed_terminal_backing_enabled=true`) proactively launches the wrapper PTY at spawn-request running transition for pi/codex/opencode/hermes managed agents so the console pre-exists before the first dispatch and subsequent sends reuse it via `_active_terminal_for_agent`. **`managed_via_wrapper=["hermes","codex"]` (new 2026-05-25)** routes managed dispatches for the listed runtimes through the `*-aify` wrapper PTY — the wrapper IS the backing. The wrapper's in-process MCP bridge claims via `executionModes=["channel","resident"]` (set by `AIFY_MANAGED_VIA_WRAPPER=1` env from bridge spawn) and delivers via the wrapper's local backing (codex app-server / hermes dashboard gateway). Dashboard Console renders the real Ink TUI via xterm.js, no synth translation. Send to an "available" managed agent → wrapper PTY auto-spawns → wrapper child bridge claims within a polling cycle → dispatch delivered. Pi is structurally excluded (omp single-client RPC). All three default off; flip per environment via `PUT /api/v1/settings` and roll back instantly. See [DECISIONS.md](DECISIONS.md) for the architectural rationale.
