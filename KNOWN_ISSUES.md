...
- **Hermes gateway liveness doesn't gate `available`.** `runtimes.js` compute-capabilities grants `resident-run` (→ agent shows `available`) whenever `runtimeConfig.gatewayUrl` is a non-empty string — `gatewayOk = !!gatewayUrl` checks PRESENCE, not reachability. So after an agent's ephemeral gateway port dies, the bridge keeps heartbeating and the agent stays `available` for the whole ~150s lease; the dispatcher accepts a run that only discovers the dead port at connect time (`ECONNREFUSED`). A reactive mitigation shipped (`mcp/stdio/hermes-managed-host.js`): on an INITIAL gateway connect-refusal the run fails with an actionable message and the agent self-corrects off `available` via the existing `/agents/{id}/resident-lost` signal (`isGatewayConnectRefused` is narrow — mid-stream/RPC errors don't trip it). The durable fix (probe gateway liveness in the capability check and/or a pre-flight reachability probe) is delicate — an async probe in the capability path plus flap risk on a momentarily-slow gateway — and is deferred to the lifecycle/status work.
