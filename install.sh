...
install_codex_turn_hooks() {
  ...
  MSYS_NO_PATHCONV=1 node -e "
    ...
    // IN PLACE: codex keys trust by a group's index, so appending ours would move the user hooks after it.
  ...
  MSYS_NO_PATHCONV=1 node "$(path_for_node "$AIFY_BRIDGE_DIR/codex-hook-trust.mjs")" "$node_hooks_file" \
    "$(path_for_node "$codex_home/config.toml")" || true
}
...
EOF
  ...
  MSYS_NO_PATHCONV=1 node -e '
    ...
    for (let i = 0; i < pairs.length; i += 2) {
      ...
      // After EITHER separator (cygpath -w on Git Bash, or an older install); it keeps aify-blocked.sh off aify-unblocked.sh.
      const script = hookCommand.split(/[\\/]/).pop().replace(/"$/, "");
      let replaced = false;
      lines = lines.map((line) => {
        const m = line.match(/^([ \t]*(?:- )?)command:[ \t]*/);
        if (m && (line.includes("/" + script) || line.includes("\\" + script))) {
          replaced = true;
