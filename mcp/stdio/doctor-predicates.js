...
// ── wrapper-current: is the installed launcher the build this checkout describes? ────
//
// WHY THIS EXISTS AT ALL. `install.sh` guarantees the wrapper and the bridge are the same build by
// generating one and copying the other in a single step. Publishing the wrappers separately (operator
// decision, 2026-08-19) ends that guarantee by construction. `bridge-installed` and `bridge-current`
// exist BECAUSE the guarantee kept being violated silently, so giving it up without a replacement
// hands back exactly the failure class this tool was built for.
//
// IT READS, IT DOES NOT RUN. Asking a wrapper its version by executing `claude-aify --check` is unsafe
// here: a wrapper installed before the contract does not know that flag and forwards it to the runtime
// as an unrecognised argument — doctor would LAUNCH CLAUDE, on a machine with a live fleet, to find out
// whether claude-aify was current. The marker in the file answers the same question and starts nothing.
//
// A MISSING MARKER IS STALE, NOT BROKEN. Every wrapper installed before v0.6 has none, which is the
// state an operator upgrading into this check is in. It must read as "re-run install.sh" — not as a
// malfunction, and not as green.
//
// @param {{repoVersion?: string, wrappers?: Array<{name: string, version: string|null}>}} input
/**
 * The version a launcher on PATH should be compared against.
 *
 * A launcher's HARNESS_WRAPPER_VERSION marker is stamped from aify-wrapper's VERSION at render time,
 * so that is the only number it can honestly be measured against. `wrapper-current` used aify-comms'
 * repo-root VERSION instead -- a different counter, in a different repo, on a different release
 * schedule. Both read 0.5.7 on 2026-08-20, which is why nothing noticed: the check was green by
 * coincidence, not by construction, and the first independent release on either side would have made
 * every host report STALE immediately after a clean install.
 *
 * An unreadable package version returns "" rather than borrowing the service's. wrapperVersionVerdict
...
export function wrapperVersionVerdict({ repoVersion = "", wrappers = [] } = {}) {
  const want = String(repoVersion || "").trim();
  if (!want) {
    return {
      ok: false,
      code: "unknown-all",
      detail: "Could not read the aify-wrapper package version, so no launcher can be compared "
        + "against it. Nothing was verified.",
      fix: "Run 'npm install' in mcp/stdio, then re-run install.sh. The launchers are rendered "
        + "from that package, so its VERSION is what a marker is measured against.",
    };
  }
  if (!wrappers.length) {
    return {
      ok: false,
      code: "unknown-all",
      detail: "Found no aify wrapper to read a version from. Nothing was verified.",
      fix: "Run install.sh for each client you use.",
    };
  }

  const stale = wrappers.filter((w) => String(w.version || "").trim() !== want);
  if (!stale.length) {
    return {
      ok: true,
      code: "ok",
      detail: `${wrappers.length} wrapper(s) on aify-wrapper ${want}: `
        + `${wrappers.map((w) => w.name).join(", ")}.`,
    };
  }

  const described = stale
    .map((w) => `${w.name} (${w.version ? w.version : "no version marker — pre-contract build"})`)
    .join(", ");
  return {
    ok: false,
    code: "stale",
    detail:
      `${stale.length} of ${wrappers.length} wrapper(s) are not on aify-wrapper ${want}: `
      + `${described}. The launcher on PATH was generated by an older install.`,
    fix: "Re-run install.sh for those clients. A wrapper is written at install time, so restarting it changes nothing.",
  };
}
