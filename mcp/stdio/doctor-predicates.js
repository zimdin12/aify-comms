...
export function wrapperVersionVerdict({ repoVersion = "", wrappers = [] } = {}) {
  ...
  if (!want) {
    return {
      ok: false,
      code: "unknown-all",
      detail: "Could not read the repo version, so no wrapper can be compared against it.",
      fix: "Check that the repo-root VERSION file exists and is readable.",
    };
  }
  if (!wrappers.length) {
    return {
  ...
  if (!stale.length) {
    return {
      ok: true,
      code: "ok",
      detail: `${wrappers.length} wrapper(s) on ${want}: ${wrappers.map((w) => w.name).join(", ")}.`,
    };
  }
  ...
  return {
    ...
    code: "stale",
    detail:
      `${stale.length} of ${wrappers.length} wrapper(s) are not on the checkout's version ${want}: `
