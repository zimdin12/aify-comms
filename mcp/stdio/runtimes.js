...
function createHermesResidentChannelController({ agentId, agentInfo, run, runtimeState, callbacks }) {
  // Resident-hermes wake via tui_gateway /api/ws. Spawned by hermes-aify
  // wrapper which runs `hermes dashboard --tui --port <P> --no-open` in the
  // background and exports HERMES_TUI_GATEWAY_URL + AIFY_HERMES_GATEWAY_URL.
  // The operator's `hermes chat --tui` Ink TUI attaches via WebSocket to
  // that gateway. This controller opens a SECOND WebSocket to the same
  // /api/ws and injects prompt.submit (idle session) or session.steer
  // (mid-run, when prompt.submit returns 4009). TeeTransport in
  // tui_gateway/transport.py mirrors dispatcher events to all attached
  // clients — operator sees the bridge-injected turn + model reply in
  // their live terminal TUI.
  const cfg = getRuntimeConfig(agentInfo);
  const gatewayUrl = String(cfg.gatewayUrl || "").trim();
