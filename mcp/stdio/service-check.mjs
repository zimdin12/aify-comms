 ...
 */
export async function checkService({ get, add, sh, repo, serverUrl, transportError }) {
  ...
  if (!health) {
    ...
    const cause = transportError()?.cause?.code;
    if (cause === "ECONNRESET" || cause === "UND_ERR_SOCKET") {
