...
// reads the claude transcript (readTranscript) and, when the conservative
// growth-keyed detector decides a turn ended, POSTs /turn-end (postTurnEnd).
// This is the backstop for a MISSED claude Stop hook (interrupt/ESC,
// MCP-continuation, crash, or a failed curl) — the cause of the sc-claude
// "stuck at turn_busy=1" symptom. The Stop hook stays the fast-path clear; this
// only fires when growth has clearly stopped and the hook didn't.
//
// ANTI-FEEDBACK-LOOP INVARIANT: this loop only ever READS transcript GROWTH
...
// derived status into a busy signal.
//
