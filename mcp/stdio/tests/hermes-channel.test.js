...
//
// The sidecar mirrors claude-channel.js: claim a dispatch run by agentId
// (executionModes include "channel"), drive the agent's PINNED hermes session
// via the api_server client (ensureSession + chatStream), capture the assistant
// reply, POST it back via /messages/send (inReplyTo = run.messageId), then PATCH
// the run delivered/answered and clear turn_busy. On chatStream failure the run
// is PATCHed to a failed state with a cause and the loop never crashes.
//
...
import { processClaimedRun, runPollCycle, buildReplyBody } from "../hermes-channel.js";
...
test("buildReplyBody uses the exact MessageSend field names", () => {
  const body = buildReplyBody("sc-hermes", SAMPLE_RUN, "build is green");
  assert.equal(body.from_agent, "sc-hermes");
  assert.equal(body.to, "sc-manager");
  assert.equal(body.inReplyTo, "msg-1");
  assert.equal(body.body, "build is green");
  assert.ok(body.subject); // subject is required by the model
});

test("processClaimedRun: ensures pinned session, chatStreams, posts reply, marks delivered", async (t) => {
  ...
  // Reply was posted to /messages/send with inReplyTo == run.messageId and the
  // assistant reply (fixture echoes "echo:<prompt>") as the content.
  const sendCall = findCall(calls, "POST", "/messages/send");
  assert.ok(sendCall, "expected a POST /messages/send");
  assert.equal(sendCall.body.from_agent, "sc-hermes");
  assert.equal(sendCall.body.to, "sc-manager");
  assert.equal(sendCall.body.inReplyTo, "msg-1");
  assert.match(String(sendCall.body.body), /^echo:/);

  // Run PATCHed to a settled (delivered/answered/completed) status.
  const patchCall = findCall(calls, "PATCH", (e) => e.startsWith("/dispatch/runs/"));
  assert.ok(patchCall, "expected a PATCH /dispatch/runs/<id>");
  assert.match(String(patchCall.body.status), /delivered|answered|completed/);
...
test("runPollCycle: claims a channel run for the agent, drives it, settles it", async (t) => {
  ...
  // The claimed run got driven + settled.
  assert.ok(findCall(calls, "POST", "/messages/send"));
  assert.ok(findCall(calls, "PATCH", (e) => e.startsWith("/dispatch/runs/")));
});
