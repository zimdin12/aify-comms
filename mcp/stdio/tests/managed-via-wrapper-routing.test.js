...
test("wrapper-backed codex: main bridge must NOT claim managed", () => {
  const info = {
    sessionMode: "managed",
    runtime: "codex",
    capabilities: ["managed-run", "native-managed-run", "resume", "interrupt", "spawn"],
  ...
  assert.deepEqual(modes, [], `wrapper-backed codex: empty modes for main bridge; got ${JSON.stringify(modes)}`);
});
