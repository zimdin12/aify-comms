...
# `resident_lost` moved to `agents/session_lease.py` in v0.5.4. A round-trip proof names the
# module holding the CALLER, so a relocation must touch it — see the one-line pin below.
CALLER = REPO / "service" / "routers" / "agents" / "session_lease.py"
...
class ResidentLostSplitIsInertTests(unittest.TestCase):
    ...
    def test_the_source_function_is_still_where_this_proof_looks(self):
        ...
        Added when `resident_lost` moved out of `session_ops.py` in v0.5.4. The round trip already
        fails then — it cannot find the caller to inline into — but it fails as a gate-internal
        error about a missing definition. This says the true thing in one line instead.
        """
        declared = {
    ...
    def test_the_helper_is_not_still_inline(self):
        ...
        for helper in EXTRACTIONS:
            self.assertNotIn(
                ...
                f"{helper} is back in session_lease.py; this proof is vacuous")
