...
#: THE HOST'S SCREEN OBSERVATION, 2026-09-14. A liveness frame may carry `activity` -- what aify-env
#: sees on the worker's screen -- and the one call that records the frame now records that too. It
#: returns the agent whose status may have moved, and the route broadcasts it after the commit: the
#: same order every other status writer uses, and still no status and no output on the terminal.
#: Undone FIRST, because the liveness block `_PROMPT_EDITS` declares is today's text only once this
#: is undone.
_ACTIVITY_NOW = chr(10).join([
