...
class ApiV2RegressionTests(FastApiTestCase):
    ...
    def test_environment_list_api_and_dashboard_render_surface(self):
        ...
        # The legacy monolith dashboard.html is retired (2026-06-30) — the single SPA is now
        # served at the origin root by the service. /api/v1/dashboard 307-redirects there so old
        # bookmarks still land. (The SPA shell itself is served by service/main.py at "/", which
        # this api-router-only test harness does not mount.)
        dashboard = self.client.get("/api/v1/dashboard", follow_redirects=False)
        self.assertIn(dashboard.status_code, (307, 308), dashboard.text)
        self.assertEqual(dashboard.headers["location"], "/")
