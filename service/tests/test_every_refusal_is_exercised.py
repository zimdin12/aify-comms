...
A refusal is `raise HTTPException(code, message)`. There are 97 with a distinctive message, and until
this session 56 of them had never been touched by a test — a surface where FOUR live defects were
found by reading, each the same shape: a 4xx NAMING a cause the branch never established. The Start
button's "no environment bridge", the restart 409's "no online environment", the console gate's
two-way split, the cold-start family.
...
def _test_tree_text() -> str:
    """The whole test tree as one blob — and never this file.
...
UNEXERCISED_REFUSAL_CEILING = 26
...
class EveryRefusalIsExercisedTests(unittest.TestCase):
    ...
    def test_the_scan_finds_the_surface_it_claims_to_measure(self):
        ...
        self.assertTrue(
            "; auto re-registration is blocked." in text,
