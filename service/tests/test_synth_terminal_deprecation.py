...
def test_synth_skipped_for_wrapper_backed_runtimes():
    ...
    settings_on = {"managed_via_wrapper": ["codex", "hermes"]}
...
def test_synth_still_created_for_pi_native_rpc_even_if_setting_mentions_pi():
    from service.routers.agents.shared import _synth_terminal_should_be_created
    settings_on = {"managed_via_wrapper": ["codex", "hermes", "pi"]}
    assert _synth_terminal_should_be_created("pi", settings_on) is True
