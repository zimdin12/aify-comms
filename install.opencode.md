...
- Delivered managed runs should answer with `comms_send(type="response", inReplyTo=...)`; dashboard chat replies use `comms_send(to="dashboard", type="response", ...)`. A reply-dispatch back to the sender also satisfies the handoff, but it is a lower-level path. Final plain text is captured in Runs as summary/diagnostics and mirrored only as fallback repair when no explicit reply is recorded.
