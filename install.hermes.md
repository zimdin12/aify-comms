...
delivers via WS `prompt.submit`; a 4009-busy race requeues until turn-end), plus a VISIBLE
...
via gateway-WS
`prompt.submit`; a 4009-busy race requeues until turn-end). The old per-agent `hermes gateway run`
...
**Mid-run insertion is intentionally disabled for Hermes.** Hermes treats active-turn injection as an interrupt, so busy work queues until turn-end; a 4009 race requeues instead of calling `session.steer`.
