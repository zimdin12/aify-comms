...
`hermes-aify` runs the operator's real Ink terminal TUI for `hermes chat`, and it exposes a local gateway the aify-comms bridge can use for live resident dispatch. The dispatch path resumes the stored Hermes session key into a bridge-owned in-memory gateway sid, submits the prompt there, and streams the reply back to aify-comms. The operator's original terminal TUI stays usable for manual work, but it should not be treated as proof that a dispatched message landed unless the dashboard run/log shows completion.
...
4. The aify-comms bridge (loaded inside `hermes chat` as an MCP server) ALSO opens a WebSocket to the same `/api/ws` (it reads `AIFY_HERMES_GATEWAY_URL` from env, written into the hermes runtime marker by `server.js`). For inbound aify-comms messages the bridge calls `session.resume` on the registered Hermes session key, then JSON-RPC `prompt.submit` (idle session) or `session.steer` (mid-run insertion, when `prompt.submit` returns code 4009 "session busy") against the fresh in-memory sid. Hermes emits real gateway events as `event` frames such as `message.delta`, `message.complete`, `tool.start`, and `tool.complete`; aify-comms translates those into run output and chat replies.

This is the closest current Hermes equivalent to Claude Code's channel delivery and the codex resident `turn/start` path: same wrapper-spawned-daemon and bridge-as-gateway-client shape, but not the same visible-terminal semantics. The external bridge cannot target the operator TUI's private short sid; it resumes the durable session key into its own sid and reports completion through aify-comms.
