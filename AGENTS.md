...
| Wake mechanism | in-process MCP server-push | `ASYMMETRY(hermes)`: `hermes-managed-host.js` finds the real session in `session.active_list`, uses WS `prompt.submit` when idle and native `session.steer` for ordinary busy sends; explicit queue and a 4009 submit race wait for turn-end | controller inject | controller inject |
| Advertised mid-turn steering | yes | yes — ordinary busy sends use native `session.steer`; explicit queue waits for turn-end | yes | yes |
