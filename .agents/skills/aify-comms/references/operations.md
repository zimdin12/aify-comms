...
- Status taxonomy: `available` (wakeable/spawnable idle; no live worker yet) / `online` (live worker idle) / `working` (turn in progress) / `idle` / `blocked` / `stale` (resident bridge expired / no usable wake handle) / `offline` / `stopped`. The `working` state is driven by a layered signal:
  - **Per-runtime turn-start hooks** (claude-aify `UserPromptSubmit` + `PostToolUse` re-pulse, codex hooks.json `UserPromptSubmit`, hermes `pre_llm_call` shell hook) → POST `/agents/{id}/turn-start` → sets `agent_turn_state.turn_busy=1`. (`Stop` is the turn-END hook, not turn-start.)
