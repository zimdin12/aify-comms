...
- Three settings shape the managed-delivery surface (Plan 4 flipped two to default-on): `insert_messages_via_console=false` (default channel-route mode; earlier name `claude_managed_channel_only=false`) switches managed Claude from PTY-input delivery to channel events (claimed by `claude-channel.js`, emitted as `<channel source="aify-comms-channel" ...>` MCP notifications — same protocol resident Claude already uses); `managed_pty_eager_spawn=true` (Plan 4 default-on, paired with `managed_terminal_backing_enabled=true`) proactively launches the wrapper PTY at spawn-request running so the console pre-exists before the first dispatch; **`managed_via_wrapper=["codex","hermes","pi"]` (Plan 4 default-on)** routes managed dispatches for the listed runtimes through the `*-aify` wrapper PTY — the wrapper IS the backing. The wrapper's child bridge claims via `executionModes=["channel","resident"]` (signaled by `AIFY_MANAGED_VIA_WRAPPER=1` env from bridge spawn) and delivers via the wrapper's local backing (codex app-server / hermes dashboard gateway / pi omp). Dashboard Console renders the real Ink TUI via xterm.js — **synth terminals (`aify://virtual-rpc/*`) are deprecated for wrapper-backed runtimes** under Plan 4; synth remains only for opencode and as a hard-failure fallback. Flip via `PUT /api/v1/settings` and roll back instantly if anything regresses. See DECISIONS.md (Plan 4 entry) for the rationale.
...
- When `insert_messages_via_console=false` (the default), managed pi/codex/opencode/hermes delivery flows through the bridge's per-runtime controllers in `mcp/stdio/controllers/` (Plan 3 of the 2026-05-25 RuntimeAdapter refactor — `PiController`, `CodexController` + per-mode subclasses, `OpencodeController`, `HermesController` + per-mode subclasses). The adapter contract owns dispatcher selection (`adapter.controllerFor(opts)`). ALL four runtimes surface a **synthesized terminal_session** so the dashboard's Console pane shows live dispatch activity (`runtime_state.virtualTerminal=true`, `command='aify://virtual-rpc/<runtime>'`):
  ...
  - **hermes managed** — `aify://virtual-rpc/hermes`. Process is per-dispatch (`hermes chat -Q -q "<prompt>"`) via `HermesSingleShotController`, or via the resident gateway/managed gateway controllers when `runtime_config.gatewayUrl` is live; conversation context lives in the wire prompt (upstream `--continue` requires pre-existing sessions and refuses on first dispatch). The synthesized terminal_session row survives across dispatches as the operator-visibility surface. `defaultHermesCommand` probes common Windows install locations (`%USERPROFILE%\AppData\Local\hermes\hermes-agent\venv\Scripts\hermes.exe`) when PATH lookup fails.
  - **codex managed** — `aify://virtual-rpc/codex`. Per-dispatch app-server connection via `CodexManagedController` (full persistent-worker pool is Phase 5 of `docs/plans/persistent-worker-status-taxonomy.md`, deferred). Controller pushes per-event frames: `▶ turn started`, `→ <itemType>` (yellow) / `✓ <itemType>` (green) for tool items, agentMessage deltas as raw text, `■ turn ended` with usage, `✗ error` red on failure.
...
| `online` | Worker alive but no handshake-complete signal yet — wrapper/RPC child is up but not confirmed ready for traffic. |
| `ready` | Handshake-complete signal emitted by the runtime's controller (Plan 4) — worker is fully initialised and idle, between `online` and `working`. |
