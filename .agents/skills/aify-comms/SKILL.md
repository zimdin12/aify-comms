...
Ordinary sends are live-delivery gated. Offline/stale/stopped/no-wake targets fail without storing a future surprise. Managed delivery is runtime-specific but dashboard-symmetric: Codex/OpenCode/Pi use native managed/steer paths when available, Claude Code starts or reuses a `claude-aify` PTY backing and submits Messenger content as a real Claude terminal turn, and Hermes uses PTY-input delivery with a delivered Work Loop contract. Browser Console attaches to the backing PTY where one exists; it is not a separate owner. Busy steer-capable targets receive ordinary sends as steer into the active run when no direct managed/PTY delivery applies; busy non-steer targets queue/merge as next-turn work. Use `queueIfBusy=true` only when you intentionally want the next-turn path.
