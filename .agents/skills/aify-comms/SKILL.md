...
- Managed Codex has a hard dispatch timeout plus a quiet-stall watchdog. If Codex accepts `turn/start` and then emits no runtime notifications for the quiet window, the bridge fails the run visibly instead of leaving it in `running` for the full hard timeout. Tune with `runtimeConfig.timeoutMs` and `runtimeConfig.quietTimeoutMs` / `runtimeConfig.silenceTimeoutMs`.
