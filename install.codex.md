...
- Managed Codex defaults to **`gpt-5.5` with `high` reasoning effort**. Configure the global default in Dashboard **Settings -> Managed Runtime Defaults**, override a specific spawn in **Environments -> Spawn Agent**, or set a specific teammate from **Team/Sessions -> Edit agent**. Repo fallback lives in `mcp/stdio/runtimes.js` (`managedCodexConfigText` / `createCodexController`).
