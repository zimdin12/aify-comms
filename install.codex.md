...
- Managed Codex defaults to **`gpt-5.5` with `high` reasoning effort**. Configure the global defaults in Dashboard **Settings -> Managed Runtime Defaults**, override a specific spawn in **Environments -> Spawn Agent**, or set a specific teammate from **Team/Sessions -> Edit agent**. Managed Codex uses a per-agent managed `CODEX_HOME`, so one teammate's model/effort config does not overwrite another's. Repo fallback lives in `mcp/stdio/runtimes.js` (`managedCodexConfigText` / `createCodexController`).
