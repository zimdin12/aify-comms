...
- `comms_send` is the normal teamwork path and is live-delivery gated for offline/stale/no-wake targets. If the target is busy and steer-capable, normal sends steer into the active run between tool calls; use `queueIfBusy=true` only for explicit next-turn delivery. Agent-reported blocked/completed states are status notes, not delivery blockers.
