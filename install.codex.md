...
- Managed Codex defaults to **`gpt-5.5` with `high` reasoning effort**. Configure the global defaults in Dashboard **Settings -> Managed Runtime Defaults**. The normal dashboard does not tune model/effort per agent. Repo fallback lives in `mcp/stdio/runtimes.js` (`managedCodexConfigText` / `createCodexController`).
