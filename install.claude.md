...
- Managed Claude Code defaults to **`opus`**. Configure the global default in Dashboard **Settings -> Managed Runtime Defaults**, override a specific spawn in **Environments -> Spawn Agent**, or set a specific teammate from **Team/Sessions -> Edit agent**. The bridge passes the selected model as a per-run `claude --model ...` flag, so one managed Claude teammate's model does not overwrite another's.
