...
- Managed Claude Code defaults to **`opus` with `high` effort**. Configure the global defaults in Dashboard **Settings -> Managed Runtime Defaults**. The bridge passes the selected model and effort as per-run `claude --model ... --effort ...` flags; the normal dashboard does not tune model/effort per agent.
