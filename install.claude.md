...
- Managed Claude Code defaults to **`opus` with `high` effort**. Configure the global defaults in Dashboard **Settings -> Managed Runtime Defaults**, override a specific spawn in **Environments -> Spawn Agent**, or set a specific teammate from **Team/Sessions -> Edit agent**. The bridge passes the selected model and effort as per-run `claude --model ... --effort ...` flags, so one managed Claude teammate's model/effort does not overwrite another's.
