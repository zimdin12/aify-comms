...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5644 tests (+10967 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 364 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1712 tests
```
...
something. **Both of them redden on an aify-env edit**, which is the direction nobody expects. **So an aify-env PLUGIN edit can
...
```bash
cd ~/projects/aify-wrapper && node --test tests/*.test.js   # 223 tests
cd ~/projects/aify-env    && npm test                          # 1699 tests, 1 skipped; `npm test`
