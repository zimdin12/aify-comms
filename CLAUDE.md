...
```bash
python -m pytest service/tests -q                      # 4926 tests (+9886 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 396 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1429 tests
```
...
cd ~/projects/aify-env    && node --test tests/*.test.js    # 437 tests, 1 skipped
```
...
not a similar one. That walk sees 527 files and none is at or over the limit (re-run 2026-08-29: the six closest are unchanged, and the population grew by 21). A walk that FORGETS to exclude
...
`install.sh` (3,049 lines on 2026-08-29, down from 4,371 once all four wrapper bodies became template files, and up from the 2,978 recorded here when that happened) and `service/new_dashboard/styles.css` (1,839) are non-test source over the
