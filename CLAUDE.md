...
```bash
python -m pytest service/tests -q                      # 4699 tests (+9382 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 377 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1334 tests
```
