...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5588 tests (+10890 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 362 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1684 tests
```
