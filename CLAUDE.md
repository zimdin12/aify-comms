...
```bash
python -m pytest service/tests scripts/tests -q -n 8 --dist loadfile # 4764 tests, 24 skipped (+10578 subtests; 21 are deprecated pi tests)
cd mcp/stdio && node tests/run-all.mjs                 # 357 suites, 2 skipped tests, 9 pi files disabled (all named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1789 tests
```
