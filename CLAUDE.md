...
```bash
python -m pytest service/tests scripts/tests -q -n 8 --dist loadfile # 5775 tests, 3 skipped (+11198 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 369 suites, 2 skipped tests (named in its output)
...
```bash
cd ~/projects/aify-wrapper && npm test                      # 445 tests, 44 skipped on Windows. `npm test`
