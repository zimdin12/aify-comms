...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5514 tests (+10802 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 359 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1667 tests
```
...
cd ~/projects/aify-env    && npm test                          # 1331 tests, 1 skipped; `npm test`
