...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5448 tests (+10688 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 395 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1636 tests
```
