...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5566 tests (+10846 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 362 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1676 tests
```
...
cd ~/projects/aify-env    && npm test                          # 1669 tests, 1 skipped; `npm test`
...
**530 files in the committed tree** (255 Python, 275 JS), none at or over the limit. TWO are
...
This walk first read 532 / 257 / 275 because two untracked files under `.monitor/` were sitting
