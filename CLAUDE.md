...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5583 tests (+10879 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 362 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1679 tests
```
...
cd ~/projects/aify-env    && npm test                          # 1677 tests, 1 skipped; `npm test`
