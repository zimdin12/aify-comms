...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5610 tests (+10901 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 362 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1702 tests
```
