...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5405 tests (+10576 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 418 suites, 1 skipped test (named in its output)
...
```bash
cd ~/projects/aify-wrapper && node --test tests/*.test.js   # 198 tests
cd ~/projects/aify-env    && npm test                          # 1078 tests, 1 skipped; `npm test`
