...
```bash
python -m pytest service/tests scripts/tests -q -n 8 --dist loadfile # 4803 tests, 24 skipped (+10617 subtests; 21 are deprecated pi tests)
...
cd mcp/stdio && node tests/run-all.mjs                 # 358 suites, 2 skipped tests, 9 pi files disabled (all named in its output)
...
cd service/new_dashboard && node --test *.test.mjs     # 1791 tests
```
