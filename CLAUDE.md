...
```bash
python -m pytest service/tests scripts/tests -q -n 8 --dist loadfile # 5703 tests, 3 skipped (+11025 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 366 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1742 tests
```
...
```bash
cd ~/projects/aify-wrapper && npm test                      # 302 tests, 1 skipped. `npm test`
...
cd ~/projects/aify-env    && npm test                          # 1743 tests, 1 skipped; `npm test`
                                                           # NOT a bare `node --test`: the script
