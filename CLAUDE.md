...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5618 tests (+10923 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 362 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1709 tests
```
...
cd ~/projects/aify-env    && npm test                          # 1683 tests, 1 skipped; `npm test`
...
-> 540 -> 541 -> 542 -> **543 on 2026-09-09** -- 264 Python and 279 JS -- as `service/new_dashboard/console-cursor.mjs`,
...
`scripts/acceptance-ledger.py` arrived
