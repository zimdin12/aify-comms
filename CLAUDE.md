...
| `client-api-key` | a service that REFUSES unauthenticated calls plus an installed client whose `aify-comms` MCP entry carries no key -- every call that runtime's agents make returns 401, and no other row says so. The agents keep registering, so `lastSeen` refreshes and every badge stays green; a mute agent is indistinguishable from an idle one, because `Last produced` only records when an agent last SENT. Claude and hermes keep their keys in DIFFERENT files (`~/.claude.json`, `~/.hermes/config.yaml`), so half a fleet can be broken while the other half is fine. Fires only on the COMBINATION, like `api-exposure`: running with no `API_KEY` is a configuration, and a keyless client is correct against such a service. Probes WITHOUT a key on purpose -- a request carrying one can only answer "yes, with a key". **Built 2026-09-07 from a diagnosis that was WRONG**: a `grep -A 20` of the hermes block found no key because the key was on line 22, and a five-day outage was inferred from that zero. The config carries a working key and the 401 that started it is still unexplained -- but the failure mode is real and was uninstrumented, so the check stayed and its own parse walks the entry to its end by indentation rather than sampling a window |
...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5512 tests (+10802 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 356 suites, 1 skipped test (named in its output)
...
```bash
cd ~/projects/aify-wrapper && node --test tests/*.test.js   # 219 tests
cd ~/projects/aify-env    && npm test                          # 1317 tests, 1 skipped; `npm test`
