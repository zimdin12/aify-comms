...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5623 tests (+10934 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 363 suites, 1 skipped test (named in its output)
...
app serves, at that method -- ten requests against 130 routes. **So an aify-env PLUGIN edit can
