...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5431 tests (+10618 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 393 suites, 1 skipped test (named in its output)
...
cd ~/projects/aify-env    && npm test                          # 1129 tests, 1 skipped; `npm test`
