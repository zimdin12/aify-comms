...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5439 tests (+10660 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 394 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1632 tests
```
...
cd ~/projects/aify-env    && npm test                          # 1148 tests, 1 skipped; `npm test`
