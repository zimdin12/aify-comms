...
```bash
python -m pytest service/tests -q -n 8 --dist loadfile # 5454 tests (+10699 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 353 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1649 tests
```
