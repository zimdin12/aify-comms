...
```bash
python -m pytest service/tests scripts/tests -q -n 8 --dist loadfile # 5709 tests, 3 skipped (+11036 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 366 suites, 1 skipped test (named in its output)
cd service/new_dashboard && node --test *.test.mjs     # 1747 tests
```
...
cd ~/projects/aify-env    && npm test                          # 1768 tests, 1 skipped; `npm test`
                                                           # NOT a bare `node --test`: the script
