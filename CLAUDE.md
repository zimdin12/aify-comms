...
```bash
python -m pytest service/tests -q                      # 3991 tests (+7845 subtests)
cd mcp/stdio && node tests/run-all.mjs                 # 318 suites
cd service/new_dashboard && node --test *.test.mjs     # 1097 tests
```
...
the authority, never the number written here. The three above were 955/219/541 and 1576 while the real
...
`install.sh` (4,370 lines) and `service/new_dashboard/styles.css` (1,844) are non-test source over the
