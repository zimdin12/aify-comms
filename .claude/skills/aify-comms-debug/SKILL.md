...
- **[references/status.md](references/status.md)** — status labels (online vs available vs idle vs working vs stale vs offline vs stopped), the `status_engine=new` event engine, `available→online` timing, derived session status, `online` with no live worker / online-but-queued ("status lied"), `online` without a console (Plan 5 C), managed claude shown `online` while thinking or flapping `online` when the Console is closed, `working` vs `online · awaiting reply`.
