...
**Codex quiet watchdog (current build).** Managed Codex runs still have a hard two-hour timeout for genuinely long work, but the bridge also treats a turn as stalled if no Codex runtime notification or stderr line is seen for the quiet timeout window after the last activity. Default quiet timeout is 5 minutes and can be changed per agent with `runtimeConfig.quietTimeoutMs` / `runtimeConfig.silenceTimeoutMs`. The run is failed cleanly, the process tree is terminated, and a required handoff is mirrored back to the sender.
