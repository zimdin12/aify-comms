...
Managed codex now keeps a persistent `codex app-server` child per agent across dispatches (mirror of managed pi and managed hermes; see DECISIONS.md 2026-05-23). The same threadId persists turn-over-turn for native conversation continuity; turn/steer mid-turn is still supported via `comms_send` with `steer=true`. The resident path (operator-typed codex-aify with a shared WebSocket `appServerUrl`) is unchanged — it's already pooled at the app-server process level.

For live Hermes, `hermes-aify --aify-agent <id> --resume <session-id>` auto-registers the resident session when a resumable Hermes session ID is known. Dashboard-managed Hermes runs through a persistent `hermes acp` JSON-RPC child per agent by default (mirror of managed pi's persistent `omp --mode rpc`); `session/update` notifications stream into a synthesized terminal feed. **With `managed_via_wrapper=["hermes"]`, managed hermes instead runs as a bridge-spawned `hermes-aify` wrapper in a node-pty** — same wrapper the operator runs from CMD, just bridge-owned. The wrapper's child bridge claims dispatches via `executionModes=["channel","resident"]` (set by `AIFY_MANAGED_VIA_WRAPPER=1` env signal) and delivers via `prompt.submit` / `session.steer` on the wrapper's local `hermes dashboard --tui` gateway. Dashboard Console renders the real Ink chat via xterm.js. Operator sends a message to an "available" managed hermes → wrapper PTY auto-spawns → ~3-15s later the wrapper's bridge claims → dispatch delivered, no message lost. See DECISIONS.md (2026-05-24/25) and install.hermes.md.
