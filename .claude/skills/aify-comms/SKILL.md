...
- Managed runtime defaults are symmetric. Managed Claude Code defaults to `opus` with `high` effort; managed Codex defaults to `gpt-5.5` with `high` reasoning effort. Dashboard Settings define the operator defaults, and teammate/spawn records can override them per agent. Claude receives model/effort as per-run `--model` / `--effort` flags; Codex uses per-agent managed `CODEX_HOME` directories so one managed Codex teammate's config does not overwrite another's. If `max` effort is accidentally sent to Codex, the bridge normalizes it to `xhigh`.
