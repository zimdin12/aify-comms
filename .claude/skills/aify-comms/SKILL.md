...
- Managed runtime model defaults are symmetric. Managed Claude Code defaults to `opus`; managed Codex defaults to `gpt-5.5` with `high` reasoning effort. Dashboard Settings define the operator defaults, and teammate/spawn records can override them per agent. Claude receives its model as a per-run `--model` flag; Codex uses per-agent managed `CODEX_HOME` directories so one managed Codex teammate's config does not overwrite another's.
