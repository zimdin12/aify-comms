...
- When `insert_messages_via_console=false` (the default), managed pi/codex/opencode delivery flows through the bridge's native RPC adapters (`createPiController`, `createCodexController`, opencode SDK) — NOT through a visible wrapper PTY. The dashboard's Console pane should show no PTY for these agents under the default. Any attached PTY is a stale leftover and should be stopped (`UPDATE terminal_sessions SET status='stopped' WHERE agent_id=... AND status='attached'`).
