...
- Status taxonomy: `available` (env online, no live worker yet) / `online` (worker alive, not fully handshaken) / `ready` (worker initialized and idle) / `working` (turn in progress) / `offline` / `idle` / `blocked` / `stopped`. The `working` state is driven by a layered signal:
