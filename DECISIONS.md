...
**claude turn-end is read STRUCTURALLY from the transcript tail, because the `Stop` hook is unreliable.** The claude `Stop` hook (`POST /turn-end`) misses on interrupt/ESC, MCP-continuations, a crash, or a failed curl. So the bridge runs a hook-independent turn-END detector (`turn-end-detector.js` + `claude-turn-end-detector.js`, started in `server.js`) that reads `adapters/claude.js` `transcriptTail` → `{lastRole, lastStopReason, pendingToolUse}` and fires `/turn-end` ONLY when the last assistant message yielded to the user (terminal `stop_reason` ∈ {`end_turn`, `stop_sequence`, `max_tokens`} with no pending `tool_use`). *Why STRUCTURAL, not the earlier growth signal:* the claude transcript grows per completed MESSAGE, not per token, and Task sub-agents write a separate `subagents/*.jsonl`, so the PARENT transcript is STATIC during a long blocking tool call, a long generation, or any sub-agent dispatch — a "stopped growing" signal false-cleared `turn_busy` mid-turn (idle while actually working), constantly for a team running parallel sub-agents. Reading the tail STRUCTURE keeps the agent `working` through all of those. The detector is CLEAR-only (it can never set `turn_busy`); a null/unreadable tail is treated as NOT-ended (never false-clear). The `Stop` hook stays the fast path; the detector is the backstop. *Consequence:* the `PostToolUse` `turn_busy` re-pulse (claude hook + `notify-check.js`) was REMOVED — claude turn hooks are `UserPromptSubmit` (start) + `Stop` (end) only — because re-arming `turn_busy` on every tool call would defeat the turn-END event. Resident hermes has no equivalent transcript and no upstream turn-END hook, so it relies on the 30m ceiling (documented caveat; KNOWN_ISSUES.md #172).
