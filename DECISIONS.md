...
**The new engine is gated behind a `status_engine` setting whose CODE default stays `old`; the live deployment sets `status_engine=new` in the DB.** `DEFAULT_SETTINGS["status_engine"] = "old"` (`api_v2.py`), so a fresh install keeps the legacy derivation. The live service has flipped the DB row to `new`. Under `new`, `_agent_status_dict_live` overrides the served status with `derive(...)` (the pure event engine in `service/status_engine.py`) for every non-manual agent and LOGS any `status-disagreement old=… new=…` — so the engine was validated against the legacy path under live traffic before (and after) the flip. Manual statuses (`stopped`/disabled) short-circuit the engine on both paths. *Why a flag, not a hard cutover:* the disagreement log is the safety net — the new value can be compared to the old on real agents without risking the served status, and the flip is a one-line DB change reversible without a deploy.
