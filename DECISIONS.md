...
**Consequence.** Dashboard reads `agent.status` and gets `available`/`online`/`working`/`offline`/`idle`/`blocked`/`stopped`. The manual override `agents.status='stopped'` still wins via `_MANUAL_STATUSES`. Legacy bookmarks/saved filters that compared to `active` no longer match — dashboard UI bake-in. Hermes specifically uses `hermes chat -Q -q --continue aify-${agentId}` per dispatch — process is short-lived but the SESSION is durable (Hermes's own session storage handles continuity); the synthesized terminal_session row survives across dispatches as the operator-visible feed. Architecture detail in `docs/plans/persistent-worker-status-taxonomy.md`.
