...
Workplan `docs/superpowers/plans/2026-06-02-managed-hermes-lifecycle-and-restart-teardown.md` made managed runtimes (hermes first, codex/pi by extension) bridge-owned with a single lifecycle owner. The non-obvious choices:

**The delivery loop is the triad's single lifecycle owner.** The managed-hermes triad (gateway host, delivery loop, console PTY) has one supervisor: the **delivery loop**. It registers its liveness heartbeat and becomes a live claimer BEFORE bringing the gateway up (so a transient gateway failure is non-fatal/retried, not a fatal `process.exit`), owns the gateway host by port, and self-exits + tears everything down (port-killing the reused gateway host) on any terminal condition (agent-deleted/410, dead gateway, gone console, release). *Why:* before this, the gateway daemon and the loop had split, unclear ownership — a daemon could outlive its loop (zombie `hermes.exe`) or a loop could keep claiming against a dead gateway. One owner with one teardown path removes both failure modes. The wrapper then **health-gates** the loop (waits for an `aify-hermes-loop-ready-<agent>` marker the loop writes only once it is a live claimer) before showing the TUI, and fails loud rather than render a TUI that can't receive work — the visible-TUI invariant means a green Console always has a live claimer behind it.
...
**`online` means deliverable (a live claimer lease), not presence.** Managed hermes joined the channel-sidecar-delivery gate, so `online` requires a live, non-superseded CLAIMER — not gateway reachability or console presence. The positive signal is an explicit **claimer lease** the delivery loop acquires when it becomes a live claimer and releases on clean teardown; a cleanly-exited loop is immediately non-deliverable instead of waiting out a staleness window. *Why:* presence-based health (a console PTY exists, the gateway answers) manufactured false `online` for workers that couldn't actually claim — the operator-observed "online but deaf" bug. A send to a deaf target (lease recorded but no longer live) now **fails fast** with an actionable reason and writes no run, so it can't pile up to `buffer_full`. Crucially, an agent that *never* recorded a lease is treated as cold-startable (NOT deaf), preserving lazy-autostart-on-send; the lease — not mere sidecar-row absence — is the disambiguator between "deaf" and "warm but hasn't polled yet." A queued-run backstop reaper (`queued_run_backstop_seconds`, 180s) covers any in-flight run stranded after the lazy-claim ambiguity resolves.
