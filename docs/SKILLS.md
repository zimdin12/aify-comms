...
- normal sends are live-delivery gated for offline/stale/stopped/no-wake targets; terminal-capable managed runtimes may start/reuse a bridge-owned PTY. Managed Claude Code starts/reuses `claude-aify`, keeps development-channel auto-confirm off unless the operator enables it, submits dashboard turns as a single bracketed paste+submit control, and remains `working` only while the tracked run has live terminal backing and is not blocked/idle. Hermes still receives PTY-input delivery through the same atomic input path. Busy steer-capable targets receive normal sends as current-run steer when no PTY delivery applies, busy non-steer targets queue/merge as next-turn work, and `queueIfBusy=true` forces explicit next-turn delivery
...
- browser Console is current behavior when an environment bridge advertises terminal/PTY support for the runtime. It attaches to the same backing PTY where one exists and should not change the identity to `cli-takeover`. Managed Claude Code uses that PTY as a `claude-aify` backing and Messenger sends are submitted as real Claude terminal turns. Direct native CLI access still uses wrapper auto-registration (`claude-aify --aify-agent <id>` / `codex-aify --aify-agent <id>` / `hermes-aify --aify-agent <id>` / `omp-aify --aify-agent <id>`) or manual `comms_register(...)`. Ownership changes only between turns for separate resident CLIs: active managed work defers resident takeover, stale resident leases can return to managed backing automatically, and reopening with `--aify-agent` switches ownership back to resident once safe.
