...
- browser CLI is a planned dashboard ownership mode, not current behavior. Skills should not imply it works until the environment bridge advertises terminal/PTY attach. Current direct CLI access uses wrapper auto-registration (`claude-aify --aify-agent <id>` / `codex-aify --aify-agent <id>` / `omp-aify --aify-agent <id>`) or manual `comms_register(...)`. Ownership changes only between turns: active managed work defers resident takeover, and stale resident leases can return to managed backing automatically.
