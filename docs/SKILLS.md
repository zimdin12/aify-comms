...
- normal sends are live-delivery gated for offline/stopped/no-wake targets; managed Claude uses `claude-aify` PTY/channel backing, managed Codex and Hermes default to bridge-owned wrapper PTYs (`managed_via_wrapper=["codex","hermes"]`), managed Pi uses persistent OMP RPC, and OpenCode uses its native controller. For wrapper-backed managed Codex/Hermes, the environment bridge starts/reuses the wrapper PTY but the wrapper's child bridge claims the queued channel run; the environment bridge must not claim it directly. Native Codex/Hermes controllers are retained as fallback/debug paths when wrapper backing is disabled or unavailable. Dashboard turns are submitted through the runtime's backing path; Claude development-channel auto-confirm stays off unless the operator enables it. A terminal-backed run remains `working` only while the tracked run has live terminal backing and is not blocked/idle. Busy steer-capable targets receive normal sends as current-run steer when no direct delivery applies, busy non-steer targets queue/merge as next-turn work, and `queueIfBusy=true` forces explicit next-turn delivery
