...
- resident mode is a deliberate visible-terminal path for runtimes with multi-client injection: start `claude-aify --aify-agent <id>`, `codex-aify --aify-agent <id>`, or `hermes-aify --aify-agent <id>` when the CLI should temporarily own live delivery; if launched without an ID, register manually from that same session. `omp-aify` / `pi-aify` can register presence and standalone operator sessions, but triggerable Pi delivery is managed RPC.
...
- agents and managers can use `comms_contracts(...)` to inspect computed Work Loop contracts before claiming an agent is silent, overdue, answered, or blocked by stale bookkeeping. It defaults to open direct contracts; channel, self-wake, missing-reply, failed, and answered audits are explicit filters. Requests, reviews, and errors are reply contracts by default; routine `info` is not unless `requireReply` is explicitly set.
- managed turns that owe a reply or update should not end silently: stdout/logs/tool output/run summaries are telemetry, not the team-visible answer. Such a turn should close with a final reply to the triggering sender, explicit `comms_send` updates for other owners/dashboard, or a self-scheduled wake when later self-work is required
- normal sends are live-delivery gated for offline/stopped/no-wake targets; managed Claude uses `claude-aify` PTY/channel backing, managed Codex uses a bridge-owned wrapper PTY with child-bridge app-server delivery, managed Hermes uses a bridge-owned wrapper PTY with channel-sidecar gateway delivery (`managed_via_wrapper=["codex","hermes"]`), managed Pi uses persistent OMP RPC, and managed OpenCode uses its native controller through the shared environment bridge (the OpenCode-local client/resident install remains disabled). For wrapper-backed managed Codex/Hermes, the environment bridge starts/reuses the wrapper PTY but does not claim the queued channel run directly. Native Codex/Hermes controllers are retained as fallback/debug paths when wrapper backing is disabled or unavailable. Dashboard turns are submitted through the runtime's backing path; managed Claude boot prompts are answered only by cursor-verified bridge rules, while dashboard terminal input stays raw. A terminal-backed run remains `working` only while the tracked run has live terminal backing and is not blocked/idle. Busy steer-capable targets receive normal sends as current-run steer when no direct delivery applies, busy non-steer targets queue/merge as next-turn work, and `queueIfBusy=true` forces explicit next-turn delivery
...
- browser Console is current behavior when an environment bridge advertises terminal/PTY or virtual-RPC support for the runtime. It attaches to the same backing stream where one exists and should not change the identity to `cli-takeover`. Managed runtime Messenger sends are backed by wrapper PTY/channel/gateway/app-server/native RPC depending on runtime; routine messages never type through the console. Explicit console input is an authenticated, audited operator recovery/control path and remains available independently of the legacy automatic console-injection setting. Direct native CLI access still uses wrapper auto-registration (`claude-aify --aify-agent <id>` / `codex-aify --aify-agent <id>` / `hermes-aify --aify-agent <id>`) or manual `comms_register(...)`. Pi wrapper registration is presence/standalone only. Ownership changes are manual: resident registration records a candidate, but operators use **Switch to resident/managed** in Sessions or Chat details to move delivery, and stale resident sends fail visibly until switched or restarted.
...
- messages that owe replies (delivered dashboard-managed runs and resident/live sessions) are answered with `comms_send(type="response", inReplyTo=..., to="<sender|dashboard>")`; completion responses, approvals, informational updates, finals, and acknowledgements with no new question/work are read context and do not receive courtesy acknowledgements. Final plain text is the agent's working output, not the reply. The `managed_reply_capture_fallback` setting (default on) auto-mirrors the run summary only when a run ends with no explicit reply
...
- Work Loop reminders should close the original contract. Reminder notices are nudges, not new obligations; an agent should not merely acknowledge an automated reminder unless the reminder itself is the task.
...
- the Sessions page is agent-centric (one row per registered agent showing its live status + current/most-recent session), and both Chat and Sessions filter by a per-status **Statuses ▾** multiselect; ended/completed/cancelled session rows are debug history surfaced through that filter rather than a separate toggle
...
Managed runtime defaults are documented in the main skill: Claude Code and Codex model fields are blank by default, which means runtime default/latest; both default to `high` effort/reasoning effort. Bridge-spawned `claude-aify` applies the global policy when its PTY starts, so restart existing managed Claude PTYs after changing model/effort. Oh My Pi uses its own runtime defaults unless model/thinking are supplied through Dashboard Runtime settings or runtime config; for Pi, blank or `default` model means no explicit `--model` override, and Pi effort maps to OMP `--thinking`. Dashboard **Settings -> Runtime** sets global operator defaults; normal dashboard spawn/agent edit flows do not tune model/effort per agent. Managed Pi active-run steering is supported through OMP's native RPC `steer` command; use `queueIfBusy=true` for next-turn delivery instead.
