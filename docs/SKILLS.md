...
- normal sends are live-delivery gated for offline/stale/stopped/no-wake targets; terminal-capable managed runtimes may start/reuse a bridge-owned PTY. Managed Claude Code starts/reuses `claude-aify`, confirms the channel prompt when needed, writes the dashboard turn into the interactive PTY, submits it with a separate Enter, and remains `working` until the reply closes the run. Hermes still receives PTY-input delivery. Busy steer-capable targets receive normal sends as current-run steer when no PTY delivery applies, busy non-steer targets queue/merge as next-turn work, and `queueIfBusy=true` forces explicit next-turn delivery
