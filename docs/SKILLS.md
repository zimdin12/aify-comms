...
- normal sends are live-delivery gated for offline/stale/stopped/no-wake targets; terminal-capable managed runtimes start/reuse bridge-owned terminal backing by default when available, with native Codex/OpenCode/Pi adapters retained as fallback paths when terminal backing is disabled or unavailable. Dashboard turns are submitted as a single bracketed paste+submit control; Claude development-channel auto-confirm stays off unless the operator enables it. A terminal-backed run remains `working` only while the tracked run has live terminal backing and is not blocked/idle. Busy steer-capable targets receive normal sends as current-run steer when no PTY delivery applies, busy non-steer targets queue/merge as next-turn work, and `queueIfBusy=true` forces explicit next-turn delivery
...
- browser Console is current behavior when an environment bridge advertises terminal/PTY support for the runtime. It attaches to the same backing PTY where one exists and should not change the identity to `cli-takeover`. Managed runtime Messenger sends are terminal-backed by default where possible; direct native CLI access still uses wrapper auto-registration (`claude-aify --aify-agent <id>` / `codex-aify --aify-agent <id>` / `hermes-aify --aify-agent <id>` / `omp-aify --aify-agent <id>`) or manual `comms_register(...)`. Ownership changes are manual: resident registration records a candidate, but operators use **Switch to resident/managed** in Sessions or Chat details to move delivery, and stale resident sends fail visibly until switched or restarted.
