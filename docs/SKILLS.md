...
- every aify-comms message (delivered dashboard-managed runs and resident/live sessions) is answered with `comms_send(type="response", inReplyTo=..., to="<sender|dashboard>")`; final plain text is the agent's working output, not the reply. The `managed_reply_capture_fallback` setting (default on) auto-mirrors the run summary only when a run ends with no explicit reply
