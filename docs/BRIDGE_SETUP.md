...
Managed runtime defaults are explicit and symmetric where the CLIs support the same concept. Managed Claude Code defaults to `opus`; managed Codex defaults to `gpt-5.5` with `high` reasoning effort. Configure operator defaults in Dashboard **Settings -> Managed Runtime Defaults**; override one spawn from **Environments -> Spawn Agent**; override an existing teammate from **Team/Sessions -> Edit agent**. If no dashboard setting or agent override exists, the repo fallback in `mcp/stdio/runtimes.js` is used. Claude receives the model as a per-run `--model` flag. Codex uses per-agent managed `CODEX_HOME` directories plus explicit thread/turn model and effort values, so one managed Codex teammate's config does not overwrite another's.
