...
Managed runtime defaults are explicit and symmetric where the CLIs support the same concept. Managed Claude Code defaults to `opus` with `high` effort; managed Codex defaults to `gpt-5.5` with `high` reasoning effort. Configure operator defaults in Dashboard **Settings -> Managed Runtime Defaults**; override one spawn from **Environments -> Spawn Agent**; override an existing teammate from **Team/Sessions -> Edit agent**. If no dashboard setting or agent override exists, the repo fallback in `mcp/stdio/runtimes.js` is used. Claude receives model and effort as per-run `--model` / `--effort` flags. Codex uses per-agent managed `CODEX_HOME` directories plus explicit thread/turn model and effort values, so one managed Codex teammate's config does not overwrite another's. Claude supports `max` effort on capable models; Codex normalizes accidental `max` to `xhigh`.
...
Managed runs should not call `comms_register`. Dashboard-managed identities are registered by the environment bridge and stored spawn/session records. Current MCP builds reject `comms_register` when `AIFY_MANAGED_DISPATCH=1` so a managed teammate cannot accidentally downgrade itself into a resident/manual identity while handling a dashboard message.
