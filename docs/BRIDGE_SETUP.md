...
Only the `aify-comms` launcher should advertise an environment. Normal Claude/Codex/OpenCode/Pi MCP client sessions use the same stdio server for tools, messaging, and resident dispatch, but they do not register themselves as spawn targets. This prevents every open agent tab from appearing as a duplicate environment.
...
Managed delivered runs use final plain text as the primary chat protocol for the current message. The bridge captures that final answer into the run summary and stores/threads it into chat, including dashboard-origin messages. This avoids depending on an extra MCP `comms_send` call from inside the managed runtime for the reply that is already being delivered. Use `comms_send(...)` from a managed run only for separate out-of-band/proactive messages, such as a later manager update to `dashboard` after the current reply is complete. In resident/live CLI sessions, inbox replies should still use `comms_send(type="response", inReplyTo=...)`.
