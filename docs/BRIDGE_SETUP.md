...
Managed Codex defaults to `gpt-5.5` with `high` reasoning effort. Configure the operator default in Dashboard **Settings -> Managed Runtime Defaults**; override one spawn from **Environments -> Spawn Agent**; override an existing teammate from **Team/Sessions -> Edit agent**. If no dashboard setting or agent override exists, the repo fallback in `mcp/stdio/runtimes.js` is used.
