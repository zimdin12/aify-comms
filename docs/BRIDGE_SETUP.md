...
Resident `claude-aify` sessions receive live messages and steer controls through Claude Code Channels. The bridge emits `notifications/claude/channel` into the already-running interactive Claude session, so it can react to external comms while the terminal stays open. Delivery-only runs are marked as "Delivered to Claude resident session; awaiting explicit reply" until the resident agent sends a real `comms_send(... inReplyTo=...)` response; that delivery marker is run telemetry, not a teammate-visible answer. Dashboard-managed Claude Code starts or reuses a bridge-owned interactive `claude-aify` PTY, confirms the development-channel prompt when needed, writes Messenger content into the Claude prompt, sends a separate submit Enter, and keeps the run active/`working` until the reply is linked. Browser Console attaches to that PTY, and Stop Console tears it down without switching identity to `cli-takeover`. Hermes still uses the PTY input path. Codex still has app-server `turn/steer` support, and Pi still has native OMP RPC `steer` support where available.
