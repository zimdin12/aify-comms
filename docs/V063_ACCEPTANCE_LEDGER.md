...
is computed with `git merge-base --is-ancestor` rather than recalled -- three rows had been bound
...
7. **The deploy, and it is THREE stale artifacts rather than one.** Each is a separate action
   and none implies the others. Re-measured 2026-09-09, each from the command beside it:

   | artifact | how it was read | what it says |
   |---|---|---|
   | the container | `GET /health` vs `git rev-parse HEAD` | serving `3e7387a6`, version `0.6.2`; HEAD is `50b0dcb2`, **206 commits ahead**, and the served build IS an ancestor of HEAD |
...
8. **What it means for the operator, in the order the steps have to happen.** `bash
   ...
   WHAT THE REBUILD WOULD SETTLE, and it is the only open question the tests cannot: the
   operator's "our browser terminal kind of lags sometimes" is UNATTRIBUTED across 346 minutes
   of guarded observation. The mechanism this version fixed is present in the build they are
   running -- `scripts/check-deployed-console-transport.py` establishes that by running the
   CONTAINER's own queue module against this version's frame-sequence test -- and the fix has
   therefore never been in front of a browser. A rebuild is what turns that from a candidate
   into an answer, in either direction.
