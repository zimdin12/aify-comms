# CLAUDE.md before the 0.7.0 slim

This is the project `CLAUDE.md` as it stood at `c66f98d4`, kept as the record of the dated incident
history, suite-count readings and per-check narratives that the 0.7.0 rewrite moved out of the
always-loaded file. It is evidence, not instruction: where it disagrees with the live
[CLAUDE.md](../../CLAUDE.md), the live file is right.

---

## (archived) aify-comms — Claude Code project notes
...
| `external-keys` | keys issued to OTHER machines (`EXTERNAL_KEYS`) that restrict nothing: set on a service with no `API_KEY`, where nothing is authenticated and the other machine can simply omit its key, or refused as malformed, which the service only logs. Read from `/health`'s `externalKeys` counts, which carry no machine names because `/health` needs no key. Green when none are issued; `unknown` when `/health` did not answer, since no evidence is not a pass. See service/api_core/external_keys.py |
...
```bash
python -m pytest service/tests scripts/tests -q -n 8 --dist loadfile # 4847 tests, 25 skipped (+10704 subtests; 21 are deprecated pi tests)
#   NEEDS fastapi >= 0.138 IN THAT INTERPRETER: the route gates walk `iter_route_contexts`, which older
#   fastapi lacks, so nine files fail to import. Match the container: fastapi 0.139.2, starlette 1.6.0.
cd mcp/stdio && node tests/run-all.mjs                 # 359 suites, 2 skipped tests, 9 pi files disabled (all named in its output)
...
cd service/new_dashboard && node --test *.test.mjs     # 1800 tests
```
...
cd ~/projects/aify-env    && npm test                          # 1948 tests, 4 skipped on Windows; `npm test`
                                                           # NOT a bare `node --test`: the script
