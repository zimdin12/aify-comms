...
|---|---|---|
| **Does a dashboard action fail silently?** | 31 direct mutating `api()` calls, plus more reached through helpers like `patchRun`; 14 mutating handlers in `click-dispatch.mjs` are fired and forgotten with no await and no catch, and the dispatcher has no wrapper | NO, COVERED. `installRejectionToast()` is wired at `boot-wiring.mjs:343` and fires exactly once (app.js imports it but never calls it), so every one of those rejections toasts. What is lost is the NAME -- "Unexpected error: HTTP 500" rather than "Stop worker failed". Wrapping fourteen call sites in the router every click passes through, for message specificity when nothing is silent, was measured and DECLINED. `asyncAction` exists for that and is currently unused; its comment now says so, because it claimed handlers "reject silently" long after they stopped. |
| **Does the aify-env seam fail open?** | `EnvClient.#request` returns `{ok:false,error}` on unreachable, wrong status and no-endpoint alike; `env-term-shim` checks `result.ok === false` and reports; `terminal-runtime.js:348` passes the `onError` the shim needs | NO. Fail-closed by construction on the newest join. The one residual: `onError` writes to the BRIDGE's stderr, so a delegated keystroke that aify-env refuses is reported where the operator may not be looking. Surfacing it to the control plane is a real change, not a cheap one. |
