...
- Claude managed-warm uses a bridge-owned interactive PTY/channel backing. It may not show up in the `claude-aify` resume picker, but the same session can be opened by ID with `claude-aify --resume <session-id>` when the backing has recorded a resume ID.
